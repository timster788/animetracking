...
const fetchCharactersSuccess = (state, characters) =>
  state
    .set(
      'charactersData',
      fromJS(characters.map(character => ({ ...character, isFetching: false })))
    )
...
export default (state: any = initialState, action: ActionType) => {
  switch (action.type) {
    case FETCH_CHARACTERS_LOADING:
      return fetchCharactersLoading(state);
    case FETCH_CHARACTERS_SUCCESS:
      return fetchCharactersSuccess(state, action.payload);
    case FETCH_CHARACTERS_FAILURE:
      return fetchCharactersFailure(state);
...
export const getCharacterDetails = (state: any, charId: any) => {
  const char = state
    .get('charactersData')
    .toJS()
    .filter(character => character.id === charId)[0];
  if (char.character) {
